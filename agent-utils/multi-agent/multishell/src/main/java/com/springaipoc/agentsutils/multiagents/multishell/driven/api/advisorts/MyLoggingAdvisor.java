package com.springaipoc.agentsutils.multiagents.multishell.driven.api.advisorts;
...
public class MyLoggingAdvisor implements BaseAdvisor {
    ...
    public @NonNull ChatClientRequest before(@NonNull ChatClientRequest chatClientRequest, @NonNull AdvisorChain advisorChain) {
        ...
        if (this.showSystemMessage) {
            ...
            sb.append("\n - SYSTEM: ").append(first(Objects.requireNonNull(chatClientRequest.prompt().getSystemMessage().getText()), 60));
        }
        ...
        for (var generation : chatClientResponse.chatResponse().getResults()) {
            ...
            if (this.showAssistantText && message.getText() != null) {
                if (StringUtils.hasText(message.getText())) {
                    sb.append("\n - TEXT: ").append(first(message.getText(), 200));
                }
            }
        }
